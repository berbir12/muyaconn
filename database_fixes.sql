-- Database schema fixes for SkillHub app
-- Run these commands to align the database with the app requirements

-- ======================================
-- DATABASE MIGRATION: BUDGET RANGE TO SINGLE BUDGET
-- ======================================

-- Add new budget column
ALTER TABLE public.tasks ADD COLUMN IF NOT EXISTS budget_new numeric;

-- Update existing tasks to use average of budget_min and budget_max
UPDATE public.tasks 
SET budget_new = CASE 
  ...
  WHEN budget_max IS NOT NULL THEN 
    budget_max
  ELSE 
    100 -- Default value if no budget info exists
END;

-- Drop old budget columns
ALTER TABLE public.tasks DROP COLUMN IF EXISTS budget_min;
ALTER TABLE public.tasks DROP COLUMN IF EXISTS budget_max;

-- Rename new budget column to budget
ALTER TABLE public.tasks RENAME COLUMN budget_new TO budget;

-- Make budget column NOT NULL
ALTER TABLE public.tasks ALTER COLUMN budget SET NOT NULL;

-- Add constraint to ensure budget is positive
ALTER TABLE public.tasks ADD CONSTRAINT tasks_budget_positive CHECK (budget > 0);
...
ALTER COLUMN services TYPE text[] USING 
  CASE 
    WHEN services IS NULL THEN ARRAY[]::text[]
    WHEN array_length(services, 1) IS NOT NULL THEN services
    ELSE ARRAY[]::text[]
  END;
